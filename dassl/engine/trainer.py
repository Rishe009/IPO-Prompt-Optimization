...
class TrainerBase:
    ...
    # To train Ipo
    def train_llm(self, start_epoch, max_epoch, start_batch_idx, end_batch_idx, instruction):
        """Generic training loops."""
        self.start_epoch = start_epoch
        self.max_epoch = max_epoch
        ...
        self.before_train()
        self.before_epoch()
        acc, entropy_value = self.run_epoch_llm(start_batch_idx, end_batch_idx, instruction)

        return acc, entropy_value
    ...
    def run_epoch_llm(self, start_batch_idx, end_batch_idx):
        raise NotImplementedError
...
class SimpleTrainer(TrainerBase):
    ...
    def train_llm(self, start_batch_idx, end_batch_idx, instruction):
        acc,entropy_value = super().train_llm(self.start_epoch, self.max_epoch, start_batch_idx, end_batch_idx, instruction)
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch_llm(self, start_batch_idx, end_batch_idx, instruction):
        self.set_model_mode("train")
        losses = MetricMeter()
        batch_time = AverageMeter()
        data_time = AverageMeter()
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            ...
            data_time.update(time.time() - end)
            acc, entropy_value = self.forward_backward_llm(batch, instruction)
        ...
        return average_acc, entropy_value
