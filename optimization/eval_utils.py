...
def evaluate_single_instruction(
    ...
    # generate raw prompts  新的instruction用于生成raw prompts 然后测试
    ...
    if evaluate_in_parallel:
        ...
        # first round of prompting to get raw answers  获取原始回答（raw answers）
    ...
    # pylint: disable=g-long-lambda
    choices = list(  # 对两个列表raw_answers_to_parse和prediction_treat_as_number_list中的对应元素进行处理，将处理结果组成一个新的列表choices。
        map(
